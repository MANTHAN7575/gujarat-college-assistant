...
export const ChatPage: React.FC = () => {
  ...
  return (
    <div className="chat-page-root min-h-screen bg-[#E2E8F0] dark:bg-[#0B0F19] text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200 h-screen overflow-hidden relative">
      ...
      >
        {/* Left Session History Sidebar Card / Mobile Drawer */}
        <aside className={`chat-history-sidebar ${mobileSidebarOpen ? "mobile-open" : ""}`}>
          <AnimatedDotCard className="p-3.5 sm:p-4 flex-1">
            <div className="flex items-center justify-between mb-3 pb-2.5 border-b border-slate-200 dark:border-slate-800">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  onClick={() => setMobileSidebarOpen(false)}
                  className="lg:hidden text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 p-1.5 rounded-lg text-xs"
                  title="Close History"
                >
                  ✕
                </button>
        ...
        <main className="flex-1 flex flex-col overflow-hidden min-w-0">
          <AnimatedDotCard topBorderAccent={true} className="flex-1 flex flex-col overflow-hidden">
            {/* Header */}
            <div className="relative z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-3 sm:p-3.5 px-4 sm:px-5 flex items-center justify-between gap-2.5">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                <Link
                  to="/"
                  className="text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-700 px-2.5 sm:px-3 py-1.5 rounded-xl transition-all shadow-xs flex items-center gap-1.5 shrink-0"
                >
                ...
                <button
                  ...
                  className="chat-sidebar-toggle-btn text-xs font-semibold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 px-2.5 py-1.5 rounded-xl transition-all shadow-xs flex items-center gap-1.5 shrink-0"
                  title="Open Chat History"
                >
                  <History className="w-3.5 h-3.5" />
                  <span className="hidden xs:inline text-[11px]">History</span>
                </button>

                <div className="min-w-0">
                  <h2 className="font-extrabold text-sm sm:text-base text-slate-900 dark:text-white leading-tight truncate">Gujarat College Assistant AI</h2>
                  <p className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate">Smart Admission Guidance • Verified Gujarat College Data</p>
                </div>
              </div>
              <span className="bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full flex items-center gap-1.5 shrink-0">
                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span> Active
              </span>
            </div>

            {/* Messages Stream */}
            <div className="relative z-10 flex-1 overflow-y-auto p-3 sm:p-5 space-y-3 sm:space-y-4 bg-slate-50/50 dark:bg-slate-950/40">
              {messages.map((msg) => (
                <div
                  key={msg.id}
                ...
                >
                  <div
                    className={`chat-bubble rounded-2xl leading-relaxed shadow-xs ${
                      msg.sender === "user"
                  ...
                  >
                    {msg.sender === "user" ? (
                      ...
                      <ReactMarkdown
                        components={{
                          h3: ({ node, ...props }) => (
                            <h3 className="font-extrabold text-sm sm:text-base text-indigo-600 dark:text-indigo-400 my-1.5" {...props} />
                          ),
                          h4: ({ node, ...props }) => (
                            <h4 className="font-bold text-xs sm:text-sm text-slate-900 dark:text-white my-1" {...props} />
                          ),
                          p: ({ node, ...props }) => (
                  ...
                  {msg.sender === "bot" && (
                    <div className="flex flex-wrap items-center gap-1.5 mt-1.5 px-1 text-xs">
                      <span className="bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium px-2 py-0.5 rounded-md text-[10px] border border-slate-200 dark:border-slate-800 flex items-center gap-1">
                        <CheckCircle2 className="w-3 h-3 text-indigo-600 dark:text-indigo-400 shrink-0" /> Verified ACPC Data
                      </span>
                      {msg.college && (
                        <span className="bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium px-2 py-0.5 rounded-md text-[10px] border border-slate-200 dark:border-slate-800 flex items-center gap-1 truncate max-w-[200px] sm:max-w-none">
                          <Building2 className="w-3 h-3 text-indigo-600 dark:text-indigo-400 shrink-0" /> {msg.college}
                        </span>
                      )}
                    </div>
                  )}
              ...
              {loading && (
                <div className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl w-max text-xs text-slate-700 dark:text-slate-300 shadow-sm animate-pulse">
                  <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-600 dark:text-indigo-400 animate-spin shrink-0" />
                  <span className="text-[11px] sm:text-xs">Finding the best college information for you...</span>
                </div>
              )}

              {/* Enhanced Interactive Error & Retry Banner */}
              {hasError && (
                <div className="bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 rounded-2xl p-3.5 sm:p-4 text-xs sm:text-sm text-amber-900 dark:text-amber-200 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500 shrink-0" />
                    <div>
                      <p className="font-bold text-xs sm:text-sm">Unable to Send Message</p>
                  ...
                  <button
                    onClick={handleRetry}
                    className="bg-amber-700 hover:bg-amber-800 text-white font-bold px-3 py-1.5 rounded-xl text-xs shadow-xs transition-all flex items-center gap-1.5 shrink-0"
                  >
            ...
            <div className="relative z-10 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 chat-prompts-container">
              <span className="text-[10px] sm:text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider shrink-0 flex items-center gap-1">
                <HelpCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-600 dark:text-indigo-400" />
              ...
              {quickPrompts.map((prompt, idx) => (
                <motion.button
                  ...
                  className="chat-prompt-chip bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/60 hover:text-indigo-600 dark:hover:text-indigo-400 text-slate-700 dark:text-slate-300 font-semibold transition-all border border-slate-200 dark:border-slate-700 shrink-0"
                >
                  {prompt}
                </motion.button>
              ))}
            ...
            {/* Bottom Input Dock */}
            <div className="relative z-10 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 chat-input-bar-container">
              <div className="chat-input-wrapper flex items-center bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-inner rounded-xl focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all px-1">
                <input
                  type="text"
                  ...
                  className="w-full bg-transparent text-slate-900 dark:text-slate-100 placeholder-slate-400 outline-none font-medium chat-input-field"
                />
              </div>
              <button
                onClick={() => handleSend()}
                disabled={loading || !inputMessage.trim()}
                className="chat-send-btn bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-semibold rounded-xl shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
              >
