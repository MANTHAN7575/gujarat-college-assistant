    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative z-10 bg-slate-900 border border-slate-800 shadow-2xl backdrop-blur-2xl rounded-3xl p-6 max-w-lg w-full max-h-[85vh] flex flex-col overflow-hidden text-white"
        >
          {/* Modal Header */}
          <div className="flex items-center justify-between border-b border-slate-800 pb-3.5 mb-3">
            <div>
              <span className="bg-slate-800 text-teal-400 text-[10px] font-bold px-2.5 py-0.5 rounded-md uppercase tracking-wider border border-slate-700">
                Live Search Engine
              </span>
            ...
            <button
              onClick={onClose}
              className="bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white p-2 rounded-xl text-xs font-bold transition-all border border-slate-700"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="relative mb-3.5">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 text-white placeholder-slate-500 rounded-xl pl-9 pr-4 py-2.5 text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all shadow-inner"
            />
            {searching && (
              <Loader2 className="absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-teal-400 animate-spin" />
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {searchResults.length === 0 ? (
              <div className="py-12 text-center bg-slate-950/60 rounded-2xl p-6 border border-slate-800">
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {col.acpc_code && (
                          <span className="bg-slate-800 text-slate-300 text-[10px] font-semibold px-1.5 py-0.5 rounded border border-slate-700">
                            #{col.acpc_code}
                          </span>
                        )}
                      </div>
                    ...
                    {isSelected ? (
                      <span className="bg-slate-800 text-slate-400 text-xs font-semibold px-3 py-1 rounded-xl shrink-0 flex items-center gap-1 border border-slate-700">
                        <Check className="w-3.5 h-3.5 text-teal-400" /> Added
                      </span>
                    ) : (
                      <button
                        onClick={() => onAddCollege(col.id)}
                        className="bg-teal-600 hover:bg-teal-700 text-white font-semibold text-xs px-3.5 py-1.5 rounded-xl shadow-xs transition-all shrink-0 flex items-center gap-1"
                      >
